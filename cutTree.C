...
void cutTree(){ 
    ...
    const std::string outFilename   = "/afs/cern.ch/work/a/apmorris/public/Lb2chicpK/signal_samples/Lb2chicpK_2011_2012_signal_cut.root";
...
// Signal mode cuts -----------------------------------------------------------------
    ...
    const std::string cuts =   "Lambda_b0_DTF_MASS_constr1>5500 && Lambda_b0_DTF_MASS_constr1<5700 && \
                                Lambda_b0_DTF_CHI2NDOF<7 && Lambda_b0_DTF_CHI2NDOF>0 && \
                                Lambda_b0_IPCHI2_OWNPV<25 && \
                                Lambda_b0_FDS>10 && \
                                kaon_IPCHI2_OWNPV>4 && \
                                proton_IPCHI2_OWNPV>4 && \
                                gamma_CL>0.03 && \
                                chi_c_M>3400 && chi_c_M<3700 && \
                                muplus_TRACK_GhostProb<0.3 && muminus_TRACK_GhostProb<0.3 && \
                                kaon_TRACK_GhostProb<0.3 && \
                                proton_TRACK_GhostProb<0.3 && \
                                min(muplus_ProbNNmu,muminus_ProbNNmu)>0.1 && \
                                (Lambda_b0_L0MuonDecision_TOS==1||Lambda_b0_L0DiMuonDecision_TOS==1) && \
                                (Lambda_b0_Hlt1DiMuonHighMassDecision_TOS==1||Lambda_b0_Hlt1DiMuonLowMassDecision_TOS==1||Lambda_b0_Hlt1TrackMuonDecision_TOS==1|| \
                                Lambda_b0_Hlt1SingleMuonHighPTDecision_TOS==1||Lambda_b0_Hlt1TrackAllL0Decision_TOS==1) && \
                                (Lambda_b0_Hlt2DiMuonDetachedDecision_TOS==1||Lambda_b0_Hlt2DiMuonDetachedJPsiDecision_TOS==1||Lambda_b0_Hlt2DiMuonDetachedHeavyDecision_TOS==1) && \
                                Lambda_b0_pi0veto==1 && \
                                kaon_ProbNNk > 0.3 && proton_ProbNNp > 0.3";
